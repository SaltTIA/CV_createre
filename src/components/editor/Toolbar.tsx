...
export function Toolbar({ saved }: ToolbarProps) {
  ...
  const handleNewVersion = () => { const name = prompt('輸入新版本名稱'); if (name && name.trim()) { saveVersion(name.trim()); setVersionName(name.trim()); } };
  const handleDeleteVersion = () => { if (versionName === 'default') return; if (confirm('確定刪除版本' + versionName + '？')) { deleteVersion(versionName); setVersionName('default'); loadVersion('default'); } };
  const handleRenameVersion = () => { if (versionName === 'default') return; const newName = prompt('修改版本名稱：', versionName); if (newName && newName.trim() && newName.trim() !== versionName) { renameVersion(versionName, newName.trim()); setVersionName(newName.trim()); } };
  ...
  return (
    <div className="bg-white/80 backdrop-blur-sm border-b border-slate-200/80 flex items-center px-3 lg:px-4 gap-1 shrink-0" style={{minHeight:'44px'}}>
      <button onClick={goHome} className="p-2 rounded-lg hover:bg-slate-100 transition-colors shrink-0" title="首頁">
        <Home size={16} className="text-slate-500" />
      </button>
      <div className="w-px h-5 bg-slate-200 hidden sm:block" />
      <button onClick={undo} disabled={!canUndo} className="p-2 rounded-lg hover:bg-slate-100 disabled:opacity-25 shrink-0" title="復原">
        <Undo2 size={16} className="text-slate-500" />
      </button>
      <button onClick={redo} disabled={!canRedo} className="p-2 rounded-lg hover:bg-slate-100 disabled:opacity-25 shrink-0" title="重做">
        <Redo2 size={16} className="text-slate-500" />
      </button>
      <div className="w-px h-5 bg-slate-200 hidden sm:block" />
      ...
      {versionName !== 'default' && (
        <>
          <button onClick={handleRenameVersion} className="p-1.5 rounded-lg text-slate-400 hover:text-slate-600 hover:bg-slate-100 shrink-0 hidden sm:block" title="修改名稱">
            <Pencil size={12} />
          </button>
          <button onClick={handleDeleteVersion} className="p-1.5 rounded-lg text-red-500 hover:bg-red-50 shrink-0 hidden sm:block" title="刪除">
            <Trash2 size={12} />
          </button>
        </>
      )}
      ...
      <button onClick={handleExport}
        className="flex items-center gap-1 text-xs sm:text-sm bg-blue-600 text-white px-2.5 sm:px-4 py-1.5 rounded-lg hover:bg-blue-700 shadow-sm font-medium shrink-0">
        <Download size={14} />
