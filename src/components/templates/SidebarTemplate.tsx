  ...
  const renderSidebar = (key: SectionKey) => {
    switch (key) {
      ...
      case 'skills': return skills.length>0 ? (
        <div key="skills"><h2 className="text-[9pt] font-bold uppercase tracking-wider mb-2 opacity-80">Skills</h2>{skills.map((s,i)=><div key={i} className="mb-1 text-[8.5pt] opacity-80"><span className="font-semibold">{s.category}: </span>{s.items}</div>)}</div>) : null;
      case 'languages': return languages.length>0 ? (
        <div key="languages"><h2 className="text-[9pt] font-bold uppercase tracking-wider mb-2 opacity-80">Languages</h2>{languages.map((l,i)=><div key={i} className="text-[8.5pt] opacity-80">{l.language} – {l.proficiency}</div>)}</div>) : null;
      default: return null;
    }
  };
  ...
  const renderMain = (key: SectionKey) => {
    switch (key) {
      case 'summary': return summary ? (
        <div key="summary"><h2 className="text-[10pt] font-bold uppercase tracking-wider mb-2" style={{color:accentColor}}>Professional Summary</h2><p className="text-[9pt] text-slate-600">{summary}</p></div>) : null;
      case 'experiences': return experiences.length>0 ? (
        <div key="experiences"><h2 className="text-[10pt] font-bold uppercase tracking-wider mb-3" style={{color:accentColor}}>Experience</h2>{experiences.map((exp,i)=>(<div key={i} className="mb-3"><div className="flex justify-between"><span className="font-semibold text-slate-800">{exp.title} – {exp.company}</span><span className="text-[8pt] text-slate-400">{fmt(exp.startDate)} – {exp.current?'Present':fmt(exp.endDate||'')}</span></div>{exp.description&&<ul className="mt-1 space-y-0.5 list-disc list-inside text-[9pt] text-slate-600">{exp.description.split('\n').filter(Boolean).map((l,j)=><li key={j}>{l}</li>)}</ul>}</div>))}</div>) : null;
      case 'education': return education.length>0 ? (
        <div key="education"><h2 className="text-[10pt] font-bold uppercase tracking-wider mb-3" style={{color:accentColor}}>Education</h2>{education.map((edu,i)=>(<div key={i} className="mb-2"><div className="flex justify-between"><span className="font-semibold text-slate-800">{edu.school}</span><span className="text-[8pt] text-slate-400">{fmt(edu.startDate)} – {edu.endDate?fmt(edu.endDate):'Present'}</span></div><div className="text-[9pt] text-slate-500">{edu.degree} in {edu.field}</div></div>))}</div>) : null;
      case 'certifications': return certifications.length>0 ? (
        <div key="certifications"><h2 className="text-[10pt] font-bold uppercase tracking-wider mb-2" style={{color:accentColor}}>Certifications</h2>{certifications.map((c,i)=><div key={i} className="text-[9pt] text-slate-600 mb-1">{c.name} – {c.issuer} ({fmt(c.date)})</div>)}</div>) : null;
      case 'projects': return projects.length>0 ? (
        <div key="projects"><h2 className="text-[10pt] font-bold uppercase tracking-wider mb-2" style={{color:accentColor}}>Projects</h2>{projects.map((p,i)=>(<div key={i} className="mb-1"><span className="font-semibold text-slate-800 text-[9.5pt]">{p.name}</span>{p.url&&<span className="text-[8pt] text-slate-400 ml-2">({p.url})</span>}{p.description&&<p className="text-[9pt] text-slate-600">{p.description}</p>}</div>))}</div>) : null;
      case 'custom': return customSections.map((s,i)=>(<div key={'custom-'+i}><h2 className="text-[10pt] font-bold uppercase tracking-wider mb-2" style={{color:accentColor}}>{s.title}</h2>{s.items.map((item,j)=><div key={j} className="mb-1 text-[9pt] text-slate-600"><span className="font-semibold">{item.heading}</span> – {item.detail}</div>)}</div>));
      default: return null;
    }
  };
