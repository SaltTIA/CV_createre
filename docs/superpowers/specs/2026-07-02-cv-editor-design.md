...
### Undo / Redo
基於 useReducer 內建歷史堆疊，提供復原和重做功能。快捷鍵 Ctrl+Z / Ctrl+Y，工具列亦有對應按鈕。歷史上限 50 步。

### 自動儲存指示
每次 dispatch 後自動寫入 localStorage，編輯器底部顯示細微的儲存狀態指示（「已儲存」），在寫入時短暫變為「儲存中…」。

### 多版本儲存
使用者可針對不同求職目標儲存多份 CV。以 localStorage key 命名空間區分（`cv-data-v1`、`cv-data-v2`），編輯器頂部顯示版本下拉選單，支援新增、切換、重新命名、刪除版本。

### 區塊排序
左側導航每個區塊 tab 附帶上下箭頭，使用者可拖曳調整區塊在 CV 上的顯示順序。順序變化即時反映在預覽中。

## 使用體驗細節
### 主題色自訂
每套模板支援一個自選 accent color。預覽區提供顏色選擇器，變更後即時套用至模板的強調色（標題底線、區塊圖示、側邊欄底色等）。實作上以 CSS 變數（`--accent`）控制，五套模板共用同一變數。

### 字體選擇
提供四種 web-safe 字體組合：Sans-Serif（預設，Inter/System）、Serif（Georgia）、Modern（Monospace 風格）、Classic（Times New Roman 風格）。下拉選單即時切換，全頁套用。

### 拼字檢查
所有文字輸入區域（textarea、input）開啟瀏覽器原生 `spellcheck` 屬性，英文內容的拼字錯誤即時顯示紅線提示。

### 經歷條列格式
經歷與專案描述欄位中，使用者逐行輸入內容，預覽時自動以 bullet point 渲染。每行一個項目符號，支援在表單中按 Enter 新增行。

### 日期選擇器
經歷與學歷的期間欄位使用日期選擇器，格式統一為 Mon YYYY（如 "Sep 2020"）。

### 表單提示
每個欄位附帶 placeholder 範例文案（英文），使用者邊看邊填，降低打字門檻。

### 空白狀態引導
若使用者直接跳過問卷進入編輯器，左右兩欄皆預先載入一組完整的假資料範例 CV，而非顯示空白表單。使用者看到的是完整渲染的一頁，只需逐區替換成自己的內容。
