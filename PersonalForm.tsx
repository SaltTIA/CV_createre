  ...
  return (
    <div className="space-y-4">
      ...
      <Field label="電話" value={personal.phone} onChange={(v) => update('phone', v)} placeholder="e.g. +852 1234 5678" />
            <div>
        <label className="block text-sm font-medium text-slate-600 mb-1">地址（點擊地圖自動填寫詳細地址）</label>
        <textarea
          ...
          spellCheck
          rows={3}
          className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-shadow placeholder:text-slate-300 resize-none"
        />
        <div className="mt-2">
          <MapPicker onAddressSelect={(addr) => update('location', addr)} initialAddress={personal.location} />
        </div>
      </div>
...
function Field({ label, value, onChange, placeholder, type = 'text' }: {
  ...
  if (touched && value) {
    ...
    if (label === '電話' && value && !/^(\+?\d{1,3}[-\s]?)?\d{6,12}$/.test(value.replace(/\s/g, ''))) {
  ...
  return (
    <div>
      ...
      <input
        ...
        spellCheck
        className={`w-full border rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 transition-shadow placeholder:text-slate-300 ${
