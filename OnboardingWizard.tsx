...
import { MapPicker } from '../forms/MapPicker';
...
export function OnboardingWizard({ onComplete }: Props) {
  ...
  const finish = () => {
    ...
    // 1. Empty entries
    experiences.forEach((e, i) => { if (e.company.trim() && !e.title.trim()) issues.push(`Experience #${i + 1}: job title is empty`); });
    experiences.forEach((e, i) => { if (!e.company.trim() && e.title.trim()) issues.push(`Experience #${i + 1}: company name is empty`); });
    educations.forEach((e, i) => { if (e.school.trim() && !e.degree.trim()) issues.push(`Education #${i + 1}: degree is empty`); });
    educations.forEach((e, i) => { if (!e.school.trim() && e.degree.trim()) issues.push(`Education #${i + 1}: school name is empty`); });

    // 2. Future dates
    experiences.forEach((e, i) => { if (e.startDate && e.startDate > thisMonth) issues.push(`Experience #${i + 1}: start date is in the future`); });
    educations.forEach((e, i) => { if (e.startDate && e.startDate > thisMonth) issues.push(`Education #${i + 1}: start date is in the future`); });

    // 3. Duplicates
    const expCompanies = experiences.map(e => e.company.trim().toLowerCase()).filter(Boolean);
    const seenExp = new Set<string>(); expCompanies.forEach(c => { if (seenExp.has(c)) issues.push(`Duplicate company: ${c}`); else seenExp.add(c); });
    const eduSchools = educations.map(e => e.school.trim().toLowerCase()).filter(Boolean);
    const seenEdu = new Set<string>(); eduSchools.forEach(s => { if (seenEdu.has(s)) issues.push(`Duplicate school: ${s}`); else seenEdu.add(s); });

    // 4. Empty skills
    if (!skillInput.trim()) issues.push('Skills section is empty');

    if (issues.length > 0 && !warnedRef.current) {
      setWarnings(issues);
    ...
    switch (step) {
      case 0: return (
        <div className="space-y-4">
          ...
          <WizInput label="姓名 *" value={fullName} onChange={setFullName} placeholder="e.g. Alex Chen" required />
          <WizInput label="Email *" value={email} onChange={setEmail} placeholder="e.g. alex@email.com" type="email" required />
          <div className="grid grid-cols-2 gap-3">
            <WizInput label="電話" value={phone} onChange={setPhone} placeholder="+852 1234 5678" />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-600 mb-1.5">地址（點擊地圖自動填寫詳細地址）</label>
            <textarea
              value={location}
              onChange={(e) => setLocation(e.target.value)}
              placeholder="Hong Kong, Kowloon, Mong Kok, 123 Nathan Road, 5/F, Flat A"
              spellCheck
              rows={3}
            ...
            />
            <div className="mt-2">
              <MapPicker onAddressSelect={(addr: string) => setLocation(addr)} initialAddress={location} />
            </div>
          </div>
        ...
        <div className="space-y-4">
          ...
          {experiences.map((exp, i) => (
            <div key={i} className="p-4 border border-slate-200 rounded-xl space-y-3 relative">
              ...
              <div className="grid grid-cols-2 gap-3">
                <WizInput label="開始日期 (選填)" type="month" value={exp.startDate} onChange={(v) => {
                  const next = [...experiences]; next[i] = { ...next[i], startDate: v }; setExperiences(next);
                }} />
                <WizInput label="結束日期 (選填)" type="month" value={exp.endDate} onChange={(v) => {
                  const next = [...experiences]; next[i] = { ...next[i], endDate: v }; setExperiences(next);
                }} />
              </div>
              {exp.startDate && exp.endDate && !exp.current && exp.endDate < exp.startDate && (
                <p className="text-[11px] text-red-500">End date must be after start date</p>
              )}
              {exp.startDate && exp.startDate > new Date().toISOString().slice(0, 7) && (
                <p className="text-[11px] text-amber-600">Start date is in the future</p>
              )}
              <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={exp.current} onChange={(e) => {
                const next = [...experiences]; next[i] = { ...next[i], current: e.target.checked, endDate: e.target.checked ? '' : next[i].endDate }; setExperiences(next);
        ...
        <div className="space-y-4">
          ...
          {educations.map((edu, i) => (
            <div key={i} className="p-4 border border-slate-200 rounded-xl space-y-3 relative">
              ...
              <div className="grid grid-cols-2 gap-3">
                <WizInput label="開始日期 (選填)" type="month" value={edu.startDate} onChange={(v) => {
                  const next = [...educations]; next[i] = { ...next[i], startDate: v }; setEducations(next);
                }} />
                <WizInput label="結束日期 (選填)" type="month" value={edu.endDate} onChange={(v) => {
                  const next = [...educations]; next[i] = { ...next[i], endDate: v }; setEducations(next);
                }} />
              </div>

              {edu.startDate && edu.endDate && edu.endDate < edu.startDate && (
                <p className="text-[11px] text-red-500">End date must be after start date</p>
              )}
              {edu.startDate && edu.startDate > new Date().toISOString().slice(0, 7) && (
                <p className="text-[11px] text-amber-600">Start date is in the future</p>
              )}
            </div>
          ))}
      ...
      case 4: return (
        <div className="space-y-4">
          ...
          <textarea value={skillInput} onChange={(e) => setSkillInput(e.target.value)}
            ...
            className="w-full border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 resize-none transition-shadow" />
          {!skillInput.trim() && (
            <p className="text-[11px] text-amber-600">A CV without skills may appear incomplete. Consider adding at least a few.</p>
          )}
        </div>
      );
      case 5: return (
        ...
        <div className="space-y-4">
          ...
          {warnings.length > 0 && (
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 space-y-2">
              <p className="text-sm font-semibold text-amber-800">Please review before finishing:</p>
              <ul className="list-disc list-inside space-y-0.5">
                {warnings.map((w, i) => <li key={i} className="text-xs text-amber-700">{w}</li>)}
              </ul>
              <p className="text-[11px] text-amber-500">These are suggestions only. You can go back to fix them, or click Finish again to proceed anyway.</p>
            </div>
          )}
...
function WizInput({ label, value, onChange, placeholder, type = 'text', required = false }: {
  ...
  if (touched && value) {
    ...
    if (label === '電話' && !/^(\+?\d{1,3}[-\s]?)?\d{6,12}$/.test(value.replace(/\s/g, ''))) {
      error = 'Invalid phone number (e.g. +852 1234 5678)';
    }
  }
  if (!error && touched && !value.trim() && required) {
  ...
  return (
    <div>
      ...
      <input type={type} value={value} onChange={(e) => onChange(e.target.value)}
        ...
        className={`w-full border rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 transition-shadow placeholder:text-slate-300 ${
